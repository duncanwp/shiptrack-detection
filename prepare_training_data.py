    ...
    for dt, image_tracks in df.groupby('datetime'):
        ...
        if (len(file02) > 0) and (len(file03) > 0):
            ...
            if (not overwrite) and os.path.isfile(image_file):
                ...
                #global_scene.load([DatasetID(name='20', modifiers=('nir_reflectance',))])
                #compositor = GenericCompositor("rgb")
                #composite = compositor([global_scene['1'], global_scene['32'], global_scene['20']])
                #img = to_image(composite)
                global_scene.load(['day_microphysics']) # This uses channels 1, 20 and 31 (which is very close to the one above but looks much better)
